...
public class CustomWinCapteur implements Initializable {
    ...
    public void clicAfficher(){
        ...
        switch((String)comboBoxAlgo.valueProperty().getValue()){
            case "Algo Aléatoire" : 
                capteur = new Capteur(temp, maj, new StrategieBorne());
                break;
            case "Algo Bornes" : 
                opt2 = new Integer(textFieldOpt2.getText());
                capteur = new Capteur(temp, maj, new StrategieBorne(opt1,opt2));
                break;
            case "Algo Limité" : 
                capteur = new Capteur(temp, maj, new StrategieLimite(temp, opt1));
                break;
            default : 
